/*\
    ...
    1.09-a    15-JUL-1999  Fix bug EstimLaplaceCenters, when N_KD ~= 0
\*/
...
static int DensNormalDiag      /* ret : 0 if OK, -1 if zero density */
	    ...
	    int                NbMiss,     /* I : total nb of missing values */
...
#endif  /* !!! block to remove !!! */


/* Called by ComputeFki() */

static void DensNormalDiag      /* ret : 0 if OK, -1 if zero density */
 (
  const DataT*       DataP,      /* I : observations */
...
static void DensLaplaceDiag      /* ret : 0 if OK, -1 if zero density */
 (
  const DataT*       DataP,      /* I : observations */
  const int          K,          /* I : number of classes */
  const ModelParaT*  ParaP ,     /* I : model parameters */
...
static void DensBernoulli        /* ret : 0 if OK, -1 if zero density */
 (
  const DataT*       DataP,      /* I : observations */
  const int          K,          /* I : number of classes */
  const ModelParaT*  ParaP ,     /* I : model parameters */
...
static void EstimSizes
 (
  ...
  const int     NbMiss,        /* I : total nb of missing values */
 ...
 )
{
    ...
    {
        case FAMILY_NORMAL:
	  DensNormalDiag( DataP, SpecP->K, ParaP, PkFki_NK, LogPkFki_NK ) ;
	  break ;

        case FAMILY_LAPLACE:
	  DensLaplaceDiag( DataP, SpecP->K, ParaP, PkFki_NK, LogPkFki_NK ) ;
	  break ;

        case FAMILY_BERNOULLI:  /*V1.07-a*/
	  DensBernoulli( DataP, SpecP->K, ParaP, PkFki_NK, LogPkFki_NK ) ;
	  break ;
...
/* ------------------------------------------------------------------- */
{
  /* Gaussian mixture density, diagonal models :
     ...
     Complete data :
     fk(xi) = [ prod_d (2*pi * vkd)^(-0.5) ] * exp( -0.5 * dist_xi_mk )
     where  dist_xi_mk = sum[d=1,Nd]( (xi(d) - mk(d))^2 ) / vkd

     Incomplete data :
     fk(xi) = [ prod_{d in oi} (2*pi * vkd)^(-0.5) ] * exp( -0.5 * dist_xi_mk )
     where   dist_xi_mk = sum[d in oi]  (xi(d) - mk(d))^2 / vk
     =>
     log fk(x) = - 0.5 sum_{d in oi}[ log(2*pi * vkd) + (xid-mkd)^2/vkd ]

  */

  int     npt   = DataP->NbPts;
  int     nd    = DataP->NbVars;
  int     nmis  = DataP->NbMiss;
  int     i ;
  int     k ;
  ...
  for ( k = 0 ; k < Nk ; k ++ ) {
    ...
    for ( d = 0 ; d < nd ; d ++ ) {
      ...
      float  disp = ParaP->Disp_KD[ k * nd + d ] ;
      float  log_2pi_disp ;

      if ( disp > EPSILON ) 
	log_2pi_disp = log( TWO_PI * disp ) ;
      else
	log_2pi_disp = -MAXFLOAT ;

      for ( i = 0 ; i < npt ; i ++ ) {

	float xid     = DataP->PointsM[ i * nd + d ] ;

	if ( ! nmis || ! isnan( xid ) ) {

	  if ( ( disp > EPSILON ) && 
	       ( LogPkFki_NK[ i * Nk + k ] != -MAXFLOAT ) ) {

	    float dif     = xid - ParaP->Center_KD[ k * nd + d ] ;
	    float logfkid = - 0.5 * ( log_2pi_disp + ( dif * dif ) / disp ) ;

	    LogPkFki_NK[ i * Nk + k ] += logfkid ;
	  }
	  else {
	    LogPkFki_NK[ i * Nk + k ] = -MAXFLOAT ;
	  }
  ...
  for ( i = 0 ; i < npt ; i ++ ) {
    for ( k = 0 ; k < Nk ; k ++ ) {
      PkFki_NK[ i * Nk + k ] = exp( LogPkFki_NK[ i * Nk + k ] )  ;
    }
  }
...
/* ------------------------------------------------------------------- */
{
  ...
  for ( k = 0 ; k < Nk ; k ++ ) {
	...
	if ( ! nmis || ! isnan( xid ) ) {
          ...
          int   absdif = 
	    abs( (int) ( xid - ParaP->Center_KD[ k * nd + d ] ) ) ;

	  if ( ( disp > EPSILON ) && 
	       ( LogPkFki_NK[ i * Nk + k ] != -MAXFLOAT ) ) {

	    float logfkid = absdif * ( log_disp - log_1_minus_disp ) 
	      + log_1_minus_disp ;

	    LogPkFki_NK[ i * Nk + k ] += logfkid ;
	  }
	  else {
	    if ( disp <= EPSILON ) /* null dispersion */ {
	      if ( absdif != 0 ) /* xid != center => prob() = 0 */ {
		LogPkFki_NK[ i * Nk + k ] = -MAXFLOAT ;
	      }
	      /* else xid == center => prob() = 1 
		 => no influence on LogPkFki_NK */
	    }
	    /* else LogPkFki_NK[ i * Nk + k ] == -MAXFLOAT   already
	       => do nothing */
	  }
	}
      } /* end for i */
...
/* ------------------------------------------------------------------- */
{
  ...
  sts = CommonGaussDiag
    ( DataP->PointsM, N, D, DataP->NbMiss, C_NK, Nk, MissMode, ParaP->Disp_KD, 
      ParaP->Center_KD, 
...
/* ------------------------------------------------------------------- */
{
  ...
  sts = CommonLaplaceDiag
    ( DataP->PointsM, DataP->SortPos_ND, N, D, DataP->NbMiss, C_NK, Nk, 
      MissMode, 
      ParaP->Disp_KD, 
      ParaP->Center_KD, 
      EmptyK_P, ParaP->NbObs_K, ParaP->NbObs_KD, ParaP->Iner_KD ) ;
...
/* ------------------------------------------------------------------- */
{
  ...
  EstimSizes( X_ND, C_NK, N, D, NbMiss, K, 
	      N_K, N_KD ) ;

  sts = EstimLaplaceCenters( X_ND, Sort_ND, C_NK, N, D, NbMiss, K, Miss, 
			     N_K, N_KD, oldcent_KD, OldDisp_KD, 
			     OldNewCen_KD, EmptyK_P ) ;

  EstimLaplaceIner( X_ND, C_NK, N, D, NbMiss, K, Miss, 
...
/* ------------------------------------------------------------------- */
{
  ...
  for ( h = 0 ; h < K ; h ++ ) {
    ...
    for ( j = 0 ; j < D ; j ++ ) {
      ...
      /* If nb observations > 0 */ /*V1.09-a*/
      if ( N_KD[ h * D + j ] > EPSILON ) /* some observed data */ {
	
	/* First, find median position of observed values 
	 */
	ComputeMedian( X_ND, Sort_ND, C_NK, N, D, NbMiss, K, h, j, N_KD[ _HJ ],
		       & imed, & cumwei, & medval ) ;
	...
	if ( N_KD[ h * D + j ] == N_K[ h ] ) /* no missing data */ {
      ...
      else /* no observed data in this class / variable */ {
	NewCen_KD[ h * D + j ] = OldCen_KD[ h * D + j ] ;

	if ( N_K[ h ] <= EPSILON ) /* empty class */ {
	  /* signal empty class and which one */
	  sts = STS_W_EMPTYCLASS ;
	  (*EmptyK_P) = h + 1 ;
	}
...
/* ------------------------------------------------------------------- */
{
  ...
  if ( (*CumweiP) > halfwei + EPSILON ) /* > : median value is here*/ {
    ...
    for ( i = (*ImedP) + 1 ;
	  ( i < N ) &&
	  ( ( NbMiss && isnan( X_ND[ Sort_ND[ i * D + J ] * D + J ] ) ) ||
...
/* ------------------------------------------------------------------- */
{
          ...
          h ++ )
    {
        ...
        /* trace of its dispersion matrix : 
           trTh = sum[i]( uih * sum[d] yi(d)Ö ) - sizeh * sum[d] mh(d)Ö */
