...
void PrintHelpVersions( FILE* F )
{
    ...
    fprintf( F , "\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "Version 1.06  (26-FEB-1999)\n" ) ;
    fprintf( F , "------------\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "The beta parameter can now be estimated in a statistically more\n" ) ;
    fprintf( F , "justified way, by maximizing the fuzzy pseudo-likelihood at each\n" ) ;
    fprintf( F , "iteration.  This computation is actually equivalent to the M-step of\n" ) ;
    fprintf( F , "Zhang's (1992) mean field EM algorithm (our algorithm differs from\n" ) ;
    fprintf( F , "Zhang's through the E-step, by allowing a sequential update of the\n" ) ;
    fprintf( F , "classification instead of Zhang's parallel fixed point mechanism).\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "The Laplace family of models can now be used instead of the normal\n" ) ;
    fprintf( F , "(Gaussian) family.  This may make the algorithm more robust to\n" ) ;
    fprintf( F , "outliers.  Indeed, the center of a Laplace distribution is estimated\n" ) ;
    fprintf( F , "as the median of the class, instead of the mean for normal\n" ) ;
    fprintf( F , "distributions.\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "Other changes in functionality:\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "- the parameters of the mixture of distributions (proportions,\n" ) ;
    fprintf( F , "  centers, dispersions) can be given by the user on the command line.\n" ) ;
    fprintf( F , "  Those given parameters can be used, whether as fixed values to be\n" ) ;
    fprintf( F , "  used throughout the algorithm ('-s mf <para>'), or as \n" ) ;
    fprintf( F , "  initial starting parameters ('-s mi <para>') ;\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "- by option '-U' the sites can be updated whether:\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "   - in a parallel way, i.e. the new classification matrix is computed\n" ) ;
    fprintf( F , "     as a whole, based on the whole old classification matrix ;\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "   - in a sequential way, i.e. each (eventually fuzzy) classification \n" ) ;
    fprintf( F , "   vector is updated based on the other classification vectors, and \n" ) ;
    fprintf( F , "   this updated classification vector is used to update the following\n" ) ;
    fprintf( F , "   classification vectors.\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "- option '-c' is modified in order to allow different convergence tests : \n" ) ;
    fprintf( F , "   - 'none' : no convergence test, do the specified number iterations\n" ) ;
    fprintf( F , "   - 'clas <maxdif>' (default) : stop if the maximum change in the \n" ) ;
    fprintf( F , "     classification matrix over the last two iterations is less than '<maxdif>'\n" ) ;
    fprintf( F , "   - 'crit <dif>' : stop if the difference in the selected criterion\n" ) ;
    fprintf( F , "   over the last two iterations is less than '<dif>'\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "- option '-C' allows to select different criteria to choose between\n" ) ;
    fprintf( F , "  different local optima of the algorithm\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "A few internal changes were also made in order to accomodate for the\n" ) ;
    fprintf( F , "new functionalities.\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "Version 1.07  (08-APR-1999)\n" ) ;
    fprintf( F , "------------\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "This version adds the Bernoulli family of distributions which is\n" ) ;
    fprintf( F , "suited to binary data (or qualitative data encoded as binary).\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "Reference :\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "Dang, M. V. and Govaert, G. (1998). Fuzzy clustering of spatial binary\n" ) ;
    fprintf( F , "data.  Kybernetika, 34(4), pp.~393--398.\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "Version 1.08  (15-JUL-1999)\n" ) ;
    fprintf( F , "------------\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "- Internal changes aim to reduce the computation time.\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "- Also internally, changed srandom/random to srand/rand to enhance\n" ) ;
    fprintf( F , "  the portability of the program\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "\n" ) ;
    fprintf( F , "\n" ) ;
